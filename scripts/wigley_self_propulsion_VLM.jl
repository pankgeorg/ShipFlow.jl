#!/usr/bin/env julia
#
# Self-propulsion via J-sweep with the VLM BladedRotor. Mirrors
# wigley_self_propulsion_scan.jl but the rotor model is now the
# lifting-surface tier (LiftingSurfaces.BladedRotor) rather than the
# uniform-thrust ActuatorDisk surrogate.
#
# Procedure:
#   for J in {0.4, 0.5, ..., 1.0}:
#     Ω = π·V∞ / (J·R)
...
const J_LIST = let s = get(ENV, "WL_J_LIST", "0.4,0.5,0.6,0.7,0.8,0.9,1.0")
    [parse(Float64, x) for x in split(s, ",")]
end
const NSTEPS_PER = parse(Int, get(ENV, "WL_NSTEPS_PER", "100"))
const AVG_FRAC   = 0.25

const OUTDIR = abspath(joinpath(@__DIR__, "..", "runs", "wigley_selfprop_VLM"))
mkpath(OUTDIR)
...
@printf "Re=%.0f  NSTEPS_PER=%d  scanning %d J values…\n" Re NSTEPS_PER length(J_LIST)
