...
# Wigley submerged volume as a function of the world-frame heave
# z_h (positive = hull rises). Analytic from the SDF:
#   V_sub = (2 B L / 3) · [(2T/3) − z_h + z_h³/(3T²)]    for |z_h| ≤ T
# At z_h = 0  →  V_sub = 4LBT/9
# At z_h = T  →  V_sub = 0  (hull fully out of water)
# At z_h = −T →  V_sub = 8LBT/9 (hull fully submerged but the formula
#                                 stops being right at z_h = −T)
function V_submerged(z_h)
    z = clamp(z_h, -T_c, T_c)
    (2 * B_c * L_c / 3) * ((2*T_c/3) - z + z^3/(3*T_c^2))
end
const V0 = V_submerged(0.0)
...
OUTDIR = abspath(joinpath(@__DIR__, "..", "runs", "heave_1dof"))
mkpath(OUTDIR)
xs = collect(1:NSTEPS)
